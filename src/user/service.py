...
class UserService:
    def __init__(self,repo=None): self.repo=repo or UserRepository(); self.validator=UserValidator(); self.audit=AuditService()
    def create(self,i,name,email=""): x=self.validator.validate(User(i,name,email)); self.repo.save(x); self.audit.record("create",i); return x
