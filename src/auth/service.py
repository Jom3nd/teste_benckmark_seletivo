...
class AuthService:
    def __init__(self,repo=None): self.repo=repo or AuthRepository(); self.validator=AuthValidator(); self.audit=AuditService()
    def create(self,i,name,email=""): x=self.validator.validate(Auth(i,name,email)); self.repo.save(x); self.audit.record("create",i); return x
