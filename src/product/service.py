...
class ProductService:
    def __init__(self,repo=None): self.repo=repo or ProductRepository(); self.validator=ProductValidator(); self.audit=AuditService()
    def create(self,i,name,email=""): x=self.validator.validate(Product(i,name,email)); self.repo.save(x); self.audit.record("create",i); return x
